...
def main():
    ...
    # train
    test_score, train_time, memory_cpu, epoch_train = train_model(model, dataset, test_evaluator, early_stop, logger, config)
    print(memory_cpu)
    print("ram mean: ", np.array(memory_cpu).mean())
    yyy = len(memory_cpu)

    # Plot memory_cpu
    # plt.figure(figsize(20,10))
    plt.plot(range(yyy), memory_cpu)
    plt.xlabel("num epoch")
    plt.ylabel("memory_cpu")
    plt.show();
    plt.savefig('memory_cpu.png')
